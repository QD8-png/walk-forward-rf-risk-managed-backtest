...
def portfolio_backtest_2024(all_assets: Dict[str, Dict[str, Any]], config: StrategyConfig, verbose=True) -> Tuple[float, float, List[float], List[pd.Timestamp]]:
    """Backtests a shared capital pool rotation portfolio over multi-asset universe under non-compounding profit-withdrawal mode."""
    ...
    for day_idx, today in enumerate(all_dates):
        ...
        equity_val = capital + pocketed_profit
    ...
    if verbose:
        print("\n" + "=" * 50)
        print("2024至今 - 多资产海选轮动 - 【落袋为安/非复利模式】组合回测报告")
        print("=" * 50)
...
def plot_portfolio_equity(dates: List[pd.Timestamp], values: List[float], asset_count: int, save_path: str = None):
    ...
    # Dynamic styles
    bg_glow_color = '#FF5E62'
    ax.plot(dates_clean, norm_values, color=bg_glow_color, linewidth=2.5, label='策略资产组合总权益 (V4落袋为安自适应版)', alpha=0.95)
    ...
    ax.set_title(f'多资产海选轮动组合总权益曲线 (落袋为安模式) (2024至今 - 共 {asset_count} 只有效股票)', fontsize=14, color='#F0F6FC', pad=20, weight='bold')
...
def main() -> None:
    ...
    plot_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "plots", "v4_portfolio_equity_non_compounding.png")
